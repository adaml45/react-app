...
const postRegion = [{ title: "Region1" }, { title: "Region2" }];
...
class CommunityList extends Component {
  ...
  selectRegionClick = e => {
    this.setState({ selectRegion: e.target.id });
    this.setState({ active: true });
    this.setState({ show: true });
  };
  changeText = (id, e) => {
  ...
  render() {
    const { postHead, postsRow, search, postStatus, postRegion } = this.state;
    return (
              ...
              <div className="rowData col-12" key={row.id} id={row.id}>
                ...
                <div className="col-2">
                  <Select
                    ...
                    selectItems={postRegion.map(region => (
                      <p
                        className={
                          region.title === row.Region
                            ? "active dropdown-item"
                            : region.title === this.state.selectRegion
                            ? " activeSelect dropdown-item"
                            : "dropdown-item"
                        }
                        key={region.title}
                        id={region.title}
                        onClick={this.selectRegionClick}
                      >
                        {region.title}
                      </p>
                    ))}
